...
class F_Related extends A_Field {
	...
	public function option($option) {
		...
		return '<tr>
				...
				<tr>
                    <th>最大显示数量：</th>
                    <td>
                    <input type="text" class="input-text" size="10" name="data[setting][option][limit]" value="'.$option['limit'].'">
					<div class="onShow">关联列表搜索结果最大显示数量，默认50条</div>
                    </td>
                </tr>';
	}
	...
	public function input($cname, $name, $cfg, $value = NULL, $id = 0) {
		...
		//
		$tpl = '<li id="files_'.$name.'_{id}" style="padding-right:10px;cursor:move;border-bottom: 1px solid #EEEEEE;"><a href="javascript:;" onclick="dr_remove_file(\''.$name.'\',\'{id}\')"><img align="absmiddle" src="'.SITE_URL.'dayrui/statics/images/b_drop.png"></a>&nbsp;{value}<input type="hidden" name="data['.$name.'][]" value="{id}"></li>';
		//
		...
		function dr_add_related_'.$name.'() {
			art.dialog.open("'. MEMBER_PATH.'index.php?c=api&m=related&site='.SITE_ID.'&module='.$module.'&limit='.intval($cfg['option']['limit']).'", {
				title: "'.$cname.'",
				opacity: 0.1,
				width: 700,
				height: 400,
