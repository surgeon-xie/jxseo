...
class Template {
    ...
    public function __construct() {
        ...
        if (IS_ADMIN) {
            ...
            if (APP_DIR) {
                $this->_mdir = APPPATH.'{templates}/'.$this->_tpl_dir.'/';
            }
    ...
    public function space($dir) {
        $this->_dir = $this->_mroot = FCPATH.'member/templates/space/'.$dir.'/';
    }
    ...
    public function get_file_name($file, $dir = NULL, $include = FALSE) {
        ...
        if (IS_ADMIN || $dir == 'admin') {
            ...
            if ($dir && $this->_mdir && is_file($this->_mdir.$file)) {
            ...
            if ($this->_mdir && @is_file($this->_mdir.$file)) {
                return $this->_mdir.$file;
            }
            $error = $this->_dir.$file;
        } elseif ($file == 'go') {
    ...
    private function handle_view_file($view_content) {
        ...
        $regex_array = array(
            ...
            '#{\s*template\s+"([\$\-_\/\w\.]+)",\s*APP_DIR\s*}#Uis',
        ...
        $replace_array = array(
            ...
            "<?php if (\$fn_include = \$this->_include(\"\\1\", \"APP_DIR\")) include(\$fn_include); ?>",
    ...
    public function list_tag($_params) {
        ...
        switch ($system['action']) {
            ...
            case 'search': // 模块的搜索
                ...
                $sql_limit = $pages = '';
                $sql_where = '`id` IN(SELECT `cid` FROM `'.$table.'_search_index` WHERE `id`="'.$param['id'].'")'; // sql的where子句
        ...
        if ($where) {
            ...
            foreach ($where as $i => $t) {
                ...
                switch ($t['adj']) {
                    ...
                    default:
                        if (strpos($t['name'], '`thumb`') && $t['value'] == 1) {
                            $string.= $join." {$t['name']}<>''";
                        } else {
                            $string.= $join.(is_numeric($t['value']) ? " {$t['name']} = ".$t['value'] : " {$t['name']} = \"".($t['value'] == "''" ? '' : $this->ci->db->escape_str($t['value']))."\"");
                        }
