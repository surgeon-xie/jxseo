...
class C_Model extends CI_Model {
    ...
    private function _where(&$select, $param) {
        ...
        unset($catid);
        $_param = array();
        $this->cache_file = md5($uri.$this->uid.SITE_ID.$this->input->ip_address().$this->input->user_agent()); // 缓存文件名称
        // 存在POST提交时，重新生成缓存文件
        if (IS_POST) {
    ...
    public function edit_extend($_data, $data) {
        ...
        if ($data[1]['status'] == 9) {
            ...
            $content['title'] = $this->content['title'];
            // 更新主表
            $this->link->where('id', (int)$_data['id'])->update($this->prefix.'_extend', array('status' => $data[1]['status']));
            // 更新审核表
            $this->link->replace($this->prefix.'_extend_verify', array(
                'id' => $_data['id'],
                'uid' => $data[1]['uid'],
    ...
    public function delete_for_id($id, $tableid) {
        ...
        // 删除收藏表
        for ($i = 0; $i < 10; $i++) {
            $this->link->where('id', $id)->delete($this->prefix.'_favorite_'.$i);
        }
        // 删除应用的相关表
        ...
        if ($form) {
            foreach ($form as $f) {
                // 删除表对应的附件
                $table = SITE_ID.'_'.APP_DIR.'_form_'.$f['table'];
                $data = $this->link
                             ->where('cid', $id)
                             ->get($table)
                             ->result_array();
