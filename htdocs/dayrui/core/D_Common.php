...
class D_Common extends CI_Controller {
    ...
    private function _init_variable() {
        ...
        if ($siteid && isset($this->SITE[$siteid]) && isset($domain[$host])) {
            ...
            if ($orthers
                && $host != $this->SITE[$siteid]['SITE_DOMAIN']
                && in_array($host, $orthers)) {
                // 判断当前域名为“其他域名”
                ...
                exit;
            } elseif (isset($this->SITE[$siteid]['SITE_MOBILE'])
        ...
        if (!IS_ADMIN) {
            if (IS_MEMBER
                ...
                && isset($MEMBER['setting']['space']['domain'])
                && $MEMBER['setting']['space']['domain']) {
                // 表示空间搜索页绑定域名的情况下
                $url = MEMBER_URL;
            }
        }
        ...
        if (!$this->mobile
            && !defined('DR_UEDITOR')
            && strpos($_SERVER['REQUEST_URI'], 'm=html') === FALSE) {
            // 会员中心的自定义域名301跳转
            if (IS_MEMBER
     ...
     * @return	NULL
     */
    protected function attachment_replace_draft($id, $cid, $eid, $related) {


        $data = $this->db
                     ->where('related', $related.'_draft-'.$id)
                     ->select('id,tableid')
                     ->get('attachment')
                     ->result_array();
        if (!$data) {
     ...
     * @return	string
     */
    public function new_field_input($field, $data = NULL, $cat = FALSE, $id = 'id') {
    ...
    public function sitemap() {
        ...
        if (!$cache_data) {
            ...
            if (APP_DIR) {
                if (in_array(APP_DIR, $module) && $this->get_cache('module-'.SITE_ID.'-'.APP_DIR, 'sitemap')) {
                    // 模块内容
    ...
    public function goto_404_page($msg) {
        ...
        $root = FCPATH.'dayrui/'.($this->template->mobile ? 'mobiles' : 'templates').'/'.SITE_TEMPLATE.'/404.html';
    ...
    public function s404() {
        $this->goto_404_page('FineCMS无法找到对应的页面('.DR_URI.'),可能您没有配置好路由规则');
    }
}
