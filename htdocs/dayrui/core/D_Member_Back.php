...
class D_Member_Back extends M_Controller {
    ...
    public function edit() {
		...
		$this->template->assign(array(
            ...
            'backurl' => $backurl ? $backurl : dr_url(APP_DIR.'/back/index'),
            'listurl' => $backurl ? $backurl : dr_url(APP_DIR.'/back/index'),
            'meta_name' => lang('mod-07'),
            'result_error' => $error,
            'category_field_url' => $this->get_cache('module-'.SITE_ID.'-'.APP_DIR, 'category_field') ? dr_url(APP_DIR.'/back/edit', array('id' => $id)) : ''
		));
