...
class CI_Session_cookie extends CI_Session_driver {
	...
	protected function _sess_read()
	{
		...
		if ($session === NULL)
		{
			log_message('debug', '会话cookie没有被发现。');
			return FALSE;
		}
		...
		if ($this->sess_encrypt_cookie === TRUE)
		{
			...
			if ($session === FALSE)
			{
				log_message('debug', '无法解密会话cookie，可能是由于一个HMAC不匹配。');
				return FALSE;
			}
		}
		else
		{
			...
			{
				log_message('debug', '会话cookie没有签署。');
				return FALSE;
			}
	 ...
	 * so it's guaranteed to update even when a fatal debug
	...
	public function _update_db()
	{
		...
		if ($this->sess_use_database === TRUE && $this->data_dirty === TRUE)
		{
			...
			if (isset($this->userdata['uid'])) {
				$set['user_id'] = (int)$this->userdata['uid'];
			}
