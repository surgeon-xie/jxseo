...
function dr_space_search_url($params = NULL, $name = NULL, $value = NULL, $urlrule = NULL) {
	...
	if (SITE_MOBILE !== TRUE && $params && $space['rewrite']) {
		return ($space['domain'] ? 'http://'.$space['domain'] : SITE_URL.'/space').'/search-'.dr_rewrite_encode($params).'.html';
	} else {
...
function dr_space_domain($uid) {
    ...
    if (!$data) {
        ...
        $data = $space['domain'];
        $ci->set_cache_data('member-space-domain-'.$uid, $data, 1110000);
    }
...
function dr_space_list_url($uid, $id, $page = FALSE) {
	...
	if ($ci->get_cache('member', 'setting', 'space', 'rewrite')) {
		if ($page) {
			return ($domain ? $domain : MEMBER_URL.$uid.'/').'category-'.$id.'-[page].html';
		}
		return ($domain ? $domain : MEMBER_URL.$uid.'/').'category-'.$id.'.html';
	} else {
		if ($page) {
			return ($domain ? $domain.'index.php?' : MEMBER_URL.'index.php?uid='.$uid.'&').'action=category&id='.$id.'&page=[page]';
		}
		return ($domain ? $domain.'index.php?' : MEMBER_URL.'index.php?uid='.$uid.'&').'action=category&id='.$id;
	}
}
...
function dr_show_seo($mod, $data, $page = 1) {
	...
	$meta_title = $cat['setting']['seo']['show_title'] ? $cat['setting']['seo']['show_title'] : '['.dr_lang('m-214', '{page}').'{join}]{title}{join}{name}{join}{modulename}{join}{SITE_NAME}';
...
function dr_show_url($mod, $data, $page = NULL) {
	...
	if (SITE_MOBILE !== TRUE && $rule && $rule['show_page'] && $rule['show']) {
		// URL模式为自定义，且已经设置规则
	...
	return (SITE_MOBILE === TRUE ? SITE_URL.$mod['dirname'].'/' : $mod['url']).'index.php?c=show&id='.$data['id'].($page ? '&page='.$page : '');
}
...
function dr_extend_url($mod, $data) {
	...
	$ci	= &get_instance();
	$rule = $ci->get_cache('urlrule', (int)$cat['setting']['urlrule'], 'value');
	if (SITE_MOBILE !== TRUE && $rule && $rule['extend']) {
		// URL模式为自定义，且已经设置规则
		$cat['pdirname'].= $cat['dirname'];
		$data['dirname'] = $cat['dirname'];
		$inputtime = isset($data['_inputtime']) ? $data['_inputtime'] : $data['inputtime'];
	...
	return (SITE_MOBILE === TRUE ? SITE_URL.$mod['dirname'].'/' : $mod['url']).'index.php?c=extend&id='.$data['id'];
}
...
function dr_category_url($mod, $data, $page = NULL) {
	...
	if (SITE_MOBILE !== TRUE && $rule && $rule['list'] && $rule['list_page']) {
		// URL模式为自定义，且已经设置规则
	...
	return (SITE_MOBILE === TRUE ? SITE_URL.$mod['dirname'].'/' : $mod['url']).'index.php?c=category&id='.$data['id'].($page ? '&page='.$page : '');
}
...
function dr_page_url($data, $page = NULL) {
	...
	if ($data['module'] && $module = get_module($data['module'])) {
		$path = SITE_MOBILE === TRUE ? SITE_URL.$module['dirname'].'/' : $module['url'];
	} else {
		$path = SITE_URL;
	}
	...
	if (SITE_MOBILE !== TRUE && $rule && $rule['page'] && $rule['page_page']) {
		// URL模式为自定义，且已经设置规则
		$data['pdirname'].= $data['dirname'];
		$data['pdirname'] = str_replace('/', $rule['catjoin'], $data['pdirname']);
