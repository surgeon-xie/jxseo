...
class Module_model extends CI_Model {
	...
	public function export_form($dir) {
		...
		if ($form) {
			$fdata = array();
			foreach ($form as $t) {
				$sql = $this->site[SITE_ID]->query("SHOW CREATE TABLE `".$this->db->dbprefix(SITE_ID.'_'.$dir.'_form_'.$t['tablename'])."`")->row_array();
	...
	public function install($id, $dir, $siteid, $config, $mydb = 0, $_siteid = 0) {
        ...
        if ($_siteid) {
            ...
            if ($form) {
                foreach ($form as $t) {
                    $sql = $db->query("SHOW CREATE TABLE `".$this->db->dbprefix($_siteid.'_'.$dir.'_form_'.$t['id'])."`")->row_array();
                    $sql = str_replace(
                        array($sql['Table'], 'CREATE TABLE'),
                        array('{tablename}', 'CREATE TABLE IF NOT EXISTS'),
                        $sql['Create Table']
                    );
                    $sql = trim(str_replace('{tablename}', $this->db->dbprefix($siteid.'_'.$dir.'_form_'.$t['id']), $sql));
                    $db->query($sql);
                }
            }
        } else {
