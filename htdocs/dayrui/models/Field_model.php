		...
		switch($this->relatedname) {
			...
			default:
				if (strpos($this->relatedname, 'form') === 0) {
                    ...
                    // 更新站点模块
                    foreach ($module as $sid => $mod) {
                        if (!in_array($this->module, $mod)) {
                            continue;
                        }
                        // 主表名称
                        $table = $this->db->dbprefix($sid.'_'.$this->module.'_form_'.$this->data);
                        // 模块表单只更新主表
                        $this->site[$sid]->query(str_replace('{tablename}', $table, $sql));
                    }
			...
			default: 
				if (strpos($this->relatedname, 'form') === 0) {
					...
					$_field	= $this->db
								   ...
								   ->where('relatedname', $this->relatedname)
								   ->count_all_results('field');
                    $_system = $tableinfo[$this->db->dbprefix(SITE_ID.'_'.$this->module.'_form_'.$this->data)]['field'];
				}  else {
					// 模块栏目分类
